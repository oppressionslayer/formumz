...
def write_report(main_agg, noise_rows, scenarios, models_order, has_main_png, has_noise_png, path):
    ...
    L("")
    L("WSRF's parity detector projects features through a newly-discovered")
    L("post-Euclid/Gauss number-theoretic boundary-detection layer, then runs")
    L("Gauss-Jordan elimination over GF(2) on the resulting binary frame. The")
    L("hidden XOR drops out of the null space; a zone-stratified forest learns")
    L("on top of the recovered structure. No exponential blow-up. Boundary")
    L("internals are proprietary, under LREL license.")
    L("")
    L("## Reproducing")
    L("")
    L("```bash")
